...
Perfetto source code, specifically the qnx-main branch that contains the new
QNX kernel probe.
...
# 3. Checkout the qnx-main branch in order to get the QNX probe sources
git checkout qnx-main
```

### Create a Perfetto build configuration for QNX Targets

Once you have the git project and the qnx-main branch you will need to create
...
```bash
# One the Linux guest we set two variables. The first is the address that relay
