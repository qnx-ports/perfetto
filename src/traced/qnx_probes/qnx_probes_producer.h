...
#ifndef SRC_TRACED_QNX_PROBES_QNX_PROBES_PRODUCER_H_
#define SRC_TRACED_QNX_PROBES_QNX_PROBES_PRODUCER_H_
...
class QnxProbesDataSource;

const uint64_t kLRUInodeCacheSize = 1000;

/**
 * Controls the lifecycle of the QNX trace probes. This class is a direct fork
 * of the src/traced/probes/ProbesProducer class.
 */
class QnxProbesProducer : public Producer {
 public:
  QnxProbesProducer();
  ~QnxProbesProducer() override;

  QnxProbesProducer(const QnxProbesProducer&) = delete;
  QnxProbesProducer& operator=(const QnxProbesProducer&) = delete;

  static QnxProbesProducer* GetInstance();
  ...
  template <typename T>
  std::unique_ptr<QnxProbesDataSource> CreateDSInstance(
 ...
 private:
  static QnxProbesProducer* instance_;
  ...
  std::unordered_map<DataSourceInstanceID, std::unique_ptr<QnxProbesDataSource>>
      data_sources_;
  ...
  // data_sources_).
  //
  // const QnxProbesDataSource::Descriptor* identifies the type.
  //
  // Used by OnFtraceDataWrittenIntoDataSourceBuffers().
  std::unordered_map<
      TracingSessionID,
      std::unordered_multimap<const QnxProbesDataSource::Descriptor*,
                              QnxProbesDataSource*>>
  ...
  base::WeakPtrFactory<QnxProbesProducer> weak_factory_;  // Keep last.
};

}  // namespace perfetto

#endif  // SRC_TRACED_QNX_PROBES_QNX_PROBES_PRODUCER_H_
