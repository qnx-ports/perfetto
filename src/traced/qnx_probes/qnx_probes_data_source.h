...
#ifndef SRC_TRACED_QNX_PROBES_QNX_PROBES_DATA_SOURCE_H_
#define SRC_TRACED_QNX_PROBES_QNX_PROBES_DATA_SOURCE_H_
...
/**
 * Base class for all data sources in traced_probes that defines the lifecycle
 * methods used by probes producer to control the data collection. This class
 * was a direct fork of src/traced/probes/ProbesDataSource with any Linux/ftrace
 * specifics redacted.
 */
class QnxProbesDataSource {
 public:
  ...
  QnxProbesDataSource(TracingSessionID, const Descriptor*);
  virtual ~QnxProbesDataSource();

  QnxProbesDataSource(const QnxProbesDataSource&) = delete;
  QnxProbesDataSource& operator=(const QnxProbesDataSource&) = delete;
...
#endif  // SRC_TRACED_QNX_PROBES_QNX_PROBES_DATA_SOURCE_H_
