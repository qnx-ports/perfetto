# Configurating traced_qnx_probes
...
```JSON
# The other perfetto configuration goes here (e.g. duration etc.)
...
| qnx_kernel_wide_events | Flag indicating whether the QNX kernel tracing should produce wide vents which contain additional data or fast events which are most concise. In fast mode we lose the priority information with the benefits of having small events which can be processed faster. | true |
...
| qnx_trace_buffer_init_bytes | The initial size of the the buffer used to hold the trace header values this dynamic buffer will grow as needed but reallocs can be avoided by selecting an initial size large enough to hold all the initial header data. | 512 |
...
data source configruation in your JSON configuration file, you will want to run
